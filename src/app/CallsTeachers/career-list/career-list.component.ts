...
Detail(row: DegreeCurriculumDesign) {
  ...
  if(this.typeUser==="Administrador"){
    this._nav.navigate(['/calls-teachers/teacher-history-list/',this.cedula]);
  }else{
    this._nav.navigate(['/calls-teachers/teacher-history-list/']);
  }
...
volverAtrasAdm(){
  this._nav.navigate(['/calls-teachers/teacher-detail/',this.cedula]);
